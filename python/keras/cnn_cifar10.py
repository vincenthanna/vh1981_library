...
from utils import sfile, skeras
...
class CNN(Model):
    def __init__(model, num_classes, in_shape=None):
    ...
    def build_model(model):
        ...
        y = Dense(num_classes, activation='softmax', name='preds')(h)
...
class Machine():
    ...
    def set_data(self, X, y):
        ...
        self.data = DataSet(X, y , num_classes)
        print("data.input_shape=", self.data.input_shape)

    def set_model(self):
        num_classes = self.num_classes
        data = self.data
        self.model = CNN(num_classes=num_classes, in_shape=data.input_shape)
    ...
    def run(self, epochs=100, batch_size=128, verbose=1):
        ...
        Y_test_pred = model.predict(data.X_test, data.Y_test, verbose=0)
