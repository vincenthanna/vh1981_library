...
CONFIGS[u'search_keywords_dict'] = {'박지성': ['박지성'],
                                    '김연아': ['김연아'],
                                    '전지현': ['전지현']}
...
def load_url_files(_dir, file_name_prefix):
    print("_dir=", _dir, " file_name_prefix=", file_name_prefix)
    url_list = []
...
def google_make_query_url(keyword, file_type, cdr):
    url = None
    query = dict(q=keyword, tbm='isch', tbs=cdr + ',ift:' + file_type)
...
def google_search_keyword(keyword, file_type, cdr):
    query_url = google_make_query_url(keyword, file_type, cdr)
...
def get_real_url(url, loaded_urls):
    ...
    try:
        ...
        print("http.client.IncompleteRead on url " + str(url))
        print(e)
...
def download_image(url, save_dir, loaded_urls=None, filetype="jpeg"):
    real_url = None
    response = None
    ...
    try:
        ...
        if loaded_urls and real_url in loaded_urls:
            ...
            img_name = hashlib.md5(real_url.encode('utf-8')).hexdigest() #경로를 hash해서 파일명으로 사용한다.
            save_image_name = os.path.join(save_dir , (img_name + '.' + filetype))
            print('Try to save image ' + real_url + ' into file: ' + save_image_name)
            output_file = open(save_image_name, 'wb')
            # read data from opened url and write to file.
            data = response.read()
    ...
    for class_name, search_keywords in CONFIGS[u'search_keywords_dict'].items():
        ...
        while True:
            ...
            while j < len(search_keywords):
                ...
                keyword = search_keywords[j]
                new_items = google_search_keyword(keyword, CONFIGS[u'search_file_type'], cdr)

                for url in new_items:
                    # real_url = get_real_url(url)
                    real_url, save_name = download_image(url, class_save_dir, loaded_urls=items, filetype=CONFIGS[u'search_file_type'])
